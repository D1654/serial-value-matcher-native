...
CommandStepExecutionResult runSerialWrite(
    ...
    result.metadata["serial_status"] = transport::serialOperationStatusName(writeResult.status);
    result.metadata["serial_error_category"] = transport::serialErrorCategoryName(writeResult.error.category);
    result.metadata["deadline_status"] = transport::serialDeadlineStatusName(writeResult.deadlineStatus);
