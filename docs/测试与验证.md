...
| GitHub Actions UI capture artifact | 最终 UI 自动证据，包含截图、`capture-status.txt`、`ui-perf-test.log` 和 `window-info.txt` | 是 |
...
需要存在并随 artifact 上传的 UI 证据至少包括：`*.png`、`capture-status.txt`、`self-test.log`、`ui-perf-test.log`、`window-info.txt`。
