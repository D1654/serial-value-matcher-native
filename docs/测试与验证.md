...
`upload-artifact` 相关门禁使用 `if-no-files-found: error`，因此关键证据缺失应直接导致 workflow 失败。需要存在并随 artifact 上传的 UI 证据至少包括：`*.png`、`capture-status.txt`、`ui-perf-test.log`、`window-info.txt`。
