...
Python/MinGW 重检按 UTF-8 文本语义比较文档，CRLF/LF 差异不会造成跨平台误报；内容变化、非法编码或异常解压文件名仍会使门禁失败并写入可读摘要。Windows PowerShell 主打包检查比较同一 checkout 内复制的原始字节。
...
- 完整 PTY normal/reopen/timeout/cancel/stress/close/reopen-generation-isolation 需要本地 Linux/Wine 执行。

`serial-pty-matrix-summary.txt`：

- 关注 `GateStatus=documented-local-only`。
- 核对 `ExpectedScenarios=normal,reopen,timeout,cancel,stress,close,reopen-generation-isolation`。
...
- 关注 `GitHubSha`、`GitHubHeadSha` 和 `CheckedOutSha`；主分支 artifact 的三个值必须与发布提交完全一致。
...
- UI capture run 的 `GitHubSha`、`GitHubHeadSha`、`CheckedOutSha` 与发布提交一致。
...
- UI capture workflow run id；
- package zip SHA256；
- package summary；
- UI evidence summary；
- 如涉及串口 I/O 改动，本地 PTY summary。

回滚时不要手工修改 zip 或复用无证据的本地 exe。应回退到上一个已验证 release，并在当前 Release 正文中说明问题和建议下载版本。

重新发布时优先递增版本号。若必须重发同一标签，Release 正文必须写明新旧 run id、新 SHA256、旧附件处理方式和修复原因。任何缺少 `Gate status: passed` 或 `GateStatus=passed` 的 artifact 都不能作为重新发布依据。
