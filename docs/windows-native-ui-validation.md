...
- 流式输出时上拉回看不会被强制跳回底部；
- 手动恢复跟随后能继续显示最新条目。
- 标签页内容区使用浅灰承托，所有单行文本输入框应与通信日志筛选/搜索输入框共享同一套边界、背景和文本边距处理。
- 输入框、下拉框和按钮在表单背景上边界清楚，不会和静态标签混成一片；截图审查时应重点比较通信日志筛选/搜索、单发输入、多发槽位和扫描页变量输入是否为同类观感。
- 同一行内标签、输入框、下拉框和按钮应在视觉上基线一致，不出现明显上下错位。
- 标签页标题、页内静态标签和占位提示中文必须清晰可读，不应因小字号、浅灰或强制抗锯齿而显得发虚。
- 小窗口下应通过控件优先级、宽度分配和低优先级标签让位处理空间不足，不应通过压薄输入框解决。
